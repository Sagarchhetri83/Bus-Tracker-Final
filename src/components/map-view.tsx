...
const MissingApiKeyAlert = () => (
    <Alert variant="destructive" className="h-[500px]">
...
export default function MapView({ selectedRouteId }: MapViewProps) {
  ...
  const center = useMemo(() => {
    if (selectedRouteId) {
      ...
      if (route && route.path.length > 0) {
        // Center on the middle point of the selected route
        const midIndex = Math.floor(route.path.length / 2);
        return route.path[midIndex];
      }
    }
    ...
    const interval = setInterval(() => {
      setBuses(prevBuses =>
        prevBuses.map(bus => {
          ...
          let newProgress = (bus.progress || 0) + 0.01;
  ...
  const visibleBuses = selectedRouteId ? buses.filter(b => b.routeId === selectedRouteId) : buses;

  return (
    <GoogleMap
      mapContainerStyle={containerStyle}
      center={center}
      zoom={selectedRouteId ? 14 : 13}
      options={{
      ...
      {routes.map(route => {
        ...
        return (
          <Polyline
            ...
            path={route.path}
            options={{
              strokeColor: isSelected ? '#EF4444' : '#4F46E5',
